        ...
        <div className="bg-[#0d1f38] border border-white/8 rounded-2xl p-8 shadow-2xl">
          {sent ? (
            <div className="text-center">
              ...
              >
                {t('backToSignIn')} <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
          ) : (
            <>
              <div className="mb-7">
                <h1 className="text-2xl font-bold text-white mb-1.5">{t('resetYourPassword')}</h1>
                <p className="text-slate-400 text-sm">{t('enterEmailToReset')}</p>
              </div>
              ...
              <form onSubmit={handleReset} className="space-y-5">
                ...
                >
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>{t('sendResetLink')} <ArrowRight className="w-4 h-4" /></>}
                </button>
              </form>

              <div className="mt-6 text-center">
                <span className="text-slate-400 text-sm">{t('rememberYourPassword')} </span>
