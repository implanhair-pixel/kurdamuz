  ...
  return (
    <div className="min-h-screen bg-[#0a1628] flex">
        ...
        <div className="p-5 border-b border-white/6">
          <Link href={`/${locale}`} className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div className="text-xs text-emerald-400">{sidebarT('subtitle')}</div>
            </div>
          </Link>
        </div>
        <nav className="flex-1 p-3 space-y-0.5">
          {[
            { icon: '📊', label: sidebarT('dashboard'), href: `/${locale}/dashboard` },
            { icon: '📖', label: sidebarT('learning'), href: `/${locale}/dashboard/learning` },
            { icon: '🎓', label: sidebarT('courses'), href: `/${locale}/courses` },
            { icon: '🎯', label: sidebarT('practice'), href: `/${locale}/srs` },
            { icon: '👥', label: sidebarT('community'), href: `/${locale}/community` },
            { icon: '🏆', label: sidebarT('leaderboard'), href: `/${locale}/leaderboard`, active: true },
            { icon: '⭐', label: sidebarT('achievements'), href: `/${locale}/achievements` },
          ].map((item) => (
            <Link
              key={item.label}
              href={item.href}
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-8">
          {loading && <div className="text-slate-400">{t('subtitle')}</div>}
                  ...
                  <div className="p-6">
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="flex-1">
                        <div className="text-sm text-slate-400">{t('rank')} #{user.rank || index + 1}</div>
                    ...
                    <div className="grid grid-cols-3 gap-3 pt-4 border-t border-white/10">
                      <div>
                        <div className="text-sm text-slate-400">{t('xp')}</div>
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-slate-400 flex items-center gap-2">
                    ...
                    <span>{t('level')} {user.currentLevel || 1}</span>
                  </div>
                </div>
