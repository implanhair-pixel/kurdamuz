...
export default async function AdminDashboardPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [totalLevelsTracked, totalCourses, totalLessons, totalXpResult, recentXpActivity] = await Promise.all([
    ...
    safe(db.select().from(xpTransactions).orderBy(desc(xpTransactions.createdAt)).limit(5), [] as any[]),
  ]);
        ...
        <div className="p-4 border-t border-white/6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-400 capitalize">{userRole === 'super_admin' ? 'Super Admin' : userRole}</div>
            </div>
          </div>
        </div>
      </aside>
