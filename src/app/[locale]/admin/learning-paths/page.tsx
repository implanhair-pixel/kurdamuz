...
export default async function AdminLearningPathsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const enrichedPaths = await Promise.all(
    paths.map(async (path) => {
      ...
      return { ...path, moduleCount: modules.length };
    })
  );
                  ...
                  <tr key={program.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex space-x-2">
                        <a href={`${basePath}/programs/${program.id}`} className="text-blue-600 hover:underline">
                          Edit
                        </a>
                        <a href={`${basePath}/programs/${program.id}/paths`} className="text-blue-600 hover:underline">
                          Paths
                        </a>
                      </div>
                    </td>
              ...
              <tbody className="divide-y divide-gray-200">
                {enrichedPaths.map((path) => (
                  <tr key={path.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                        path.active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                      }`}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex space-x-2">
                        <a href={`${basePath}/paths/${path.id}`} className="text-blue-600 hover:underline">
                          Edit
                        </a>
                        <a href={`${basePath}/paths/${path.id}/modules`} className="text-blue-600 hover:underline">
                          Modules
                        </a>
                      </div>
                    </td>
