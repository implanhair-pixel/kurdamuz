...
const DEV_MOCK_USER: DevMockUser = {
  id: 'dev-mock-user-id',
...
export async function requireRole(role: 'owner' | 'admin_super' | 'teacher' | 'student') {
  const user = await requireAuth();

  // Role is read live from the `profiles` table — see getLiveUserRole below
  // requireMinRole for why we don't trust JWT user_metadata for this check.
  const userRole = await getLiveUserRole(user);
 ...
 *
 * IMPORTANT: role is read LIVE from the `profiles` table on every call,
 * not from the JWT's user_metadata. JWT metadata is only refreshed on
 * token refresh / re-login, so a role change made directly in the
 * database (e.g. by an owner promoting a teacher to admin_super) would
 * not take effect until the affected user's session refreshed if we read
 * it from the token. Reading from `profiles` means role changes apply
 * immediately on the next request.
 *
 * Falls back to user_metadata only if no profiles row exists yet
 * (e.g. the auto-create trigger hasn't run, which should not normally
 * happen — see drizzle/0021_phase_profiles.sql).
 */
export async function requireMinRole(minRole: 'owner' | 'admin_super' | 'teacher' | 'student') {
  const user = await requireAuth();
  const userRole = await getLiveUserRole(user);
...
/**
 * Reads a user's role live from the `profiles` table.
 *
 * `profiles.role` and the RBAC hierarchy here both use the same 4-tier
 * product scheme: student | teacher | admin_super | owner. There is no
 * legacy elevated tiers anymore — every previous
 * call site that required at least 'admin' now requires 'admin_super',
 * which is the single elevated-staff tier below 'owner'.
 */
async function getLiveUserRole(user: { id: string; user_metadata?: { role?: string } }): Promise<string> {
  try {
    ...
    // If the database is unavailable, fall through to a conservative default.
  }

  // Only the injected local-dev mock user should inherit its metadata role.
  if (user.id === DEV_MOCK_USER.id) {
    return user.user_metadata?.role || 'student';
  }

  // In production, missing profile data should never escalate privileges.
  return 'student';
}
...
export async function getCurrentUserRole(): Promise<string | null> {
  const user = await getCurrentUser();
  if (!user) return null;
  return getLiveUserRole(user as { id: string; user_metadata?: { role?: string } });
}
...
export async function requirePermission(permission: VocabularyPermission) {
  const user = await requireAuth();
  const userRole = await getLiveUserRole(user as { id: string; user_metadata?: { role?: string } });
