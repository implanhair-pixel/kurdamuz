...
export class DialectSearchService {
  ...
  async searchLexicalEntries(params: DialectSearchParams): Promise<SearchResult[]> {
    ...
    if (query) {
      baseQuery = (baseQuery as unknown as typeof baseQuery).where(
        or(
    ...
    if (conditions.length > 0) {
      baseQuery = (baseQuery as unknown as typeof baseQuery).where(and(...conditions));
    }
    ...
    for (const entry of entries) {
      ...
      results.push({
        ...
        dialects: entryDialects as SearchResult['dialects'],
        variants: variants as SearchResult['variants'],
        meanings: meanings as SearchResult['meanings'],
        relevanceScore,
      });
    }
  ...
  private async getVariantsForEntry(entryId: string, dialectIds?: string[]) {
    ...
    if (dialectIds && dialectIds.length > 0) {
      query = (query as unknown as typeof query).where(inArray(dialectVariants.dialectId, dialectIds));
    }
  ...
  private async getMeaningsForEntry(entryId: string, semanticDomains?: string[]) {
    ...
    if (semanticDomains && semanticDomains.length > 0) {
      query = (query as unknown as typeof query).where(inArray(lexicalMeanings.semanticDomain, semanticDomains));
    }
