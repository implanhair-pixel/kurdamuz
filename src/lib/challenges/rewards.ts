...
import { RewardType, type ChallengeReward } from '@/types/challenges';
import { awardXP } from '@/lib/xp/xp';
import { awardAchievement } from '@/lib/xp/achievements';
...
export async function claimReward(rewardId: string, userId: string): Promise<ChallengeReward> {
  try {
...
async function applyReward(reward: ChallengeReward): Promise<void> {
  try {
    switch (reward.rewardType) {
      case RewardType.XP: {
        const amount = typeof reward.rewardValue?.amount === 'number' ? reward.rewardValue.amount : 0;
        if (amount > 0) {
          await awardXP(
            reward.userId,
            amount,
      ...
      case RewardType.ACHIEVEMENT: {
        const achievementId = typeof reward.rewardValue?.achievementId === 'string'
          ? reward.rewardValue.achievementId
          : null;
        if (achievementId) {
          await awardAchievement(reward.userId, achievementId, reward.userId);
        }
        break;
      }
    ...
    console.warn('Failed to apply challenge reward side-effect:', error);
  }
}
