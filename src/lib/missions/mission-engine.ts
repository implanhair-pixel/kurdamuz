...
export class MissionEngine {
  ...
  async getAvailableMissions(userId: string): Promise<MissionDefinition[]> {
    ...
    // Filter out missions the user has already completed
    const completedMissionIds = await this.getCompletedMissionIds(userId);
    
    return activeSchedules
      .filter(s => !completedMissionIds.includes(s.definition.id))
      .map(s => s.definition as MissionDefinition);
  }
  ...
  async updateMissionProgress(
    ...
    const isCompleted = await this.progressTracker.checkCompletion(
      mission,
      progressValue
    );
  ...
  async completeMission(userId: string, missionId: string): Promise<void> {
    ...
    // Update user mission with awarded rewards
    await db
      .update(userMissions)
      .set({
  ...
  /**
   * Get completed mission IDs for a user
   */
